...
def spy_on_session(session, placebo_path):
...
def prepare_pills(mode, placebo_path, dynamo_db_session):
    ...
    if not hasattr(this, 'pill') or not hasattr(this, 'boto3_pill'):
        ...
        boto3_pill = spy_on_session(boto3_session, placebo_path)
        boto3_pill_method = getattr(boto3_pill, mode)
        boto3_pill_method()
        pill = spy_on_session(dynamo_db_session, placebo_path)
...
def expected_response_body(directory, expectation_file, actual_response):
    file_path = '/'.join([directory, expectation_file])
